...
# Extract weather information using grep, sed, and awk
temperature=$(echo $weather | grep -o '"temp":[^,]*' | sed 's/"temp"://')
description=$(echo $weather | grep -o '"description":"[^"]*' | sed 's/"description":"//' | sed 's/"//')
